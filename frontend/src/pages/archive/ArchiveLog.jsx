...
const ArchiveLog = () => {
  ...
  const handleCaseClick = (entry) => {
    ...
    switch (caseType) {
      case 'primary':
        navigate(`/cases/primary/${caseId}`)
        break
      case 'appeal':
        navigate(`/cases/appeal/${caseId}`)
        break
      case 'supreme':
        navigate(`/cases/supreme/${caseId}`)
        break
      default:
  ...
  const endItem = Math.min(currentPage * itemsPerPage, filteredEntries.length || totalItems)
          ...
          <>
            ...
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between">
              <div className="text-sm text-slate-500 dark:text-slate-400">
                ...
                <span className="font-medium text-slate-900 dark:text-white">{filteredEntries.length || totalItems}</span> نتيجة
              </div>
